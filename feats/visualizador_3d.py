...
class AbaVisualizadorSTL(ctk.CTkFrame):
    def __init__(self, master):
        ...
        # --- INFO BOX (Cálculos de Engenharia) ---
    ...
    def visualizar_stl(self, caminho):
        try:
            # 1. Processamento Matemático com Trimesh (Evita bugs de interface)
            self.mesh_atual = trimesh.load(caminho)
            
            # Cálculo de volume e peso
            vol_cm3 = self.mesh_atual.volume / 1000
            peso_estimado = vol_cm3 * 1.25 # Densidade média PLA
            ...
            # 2. Renderização OpenGL Nativa (Resolve o erro de _actors)
            # Usar pv.Plotter() é mais estável que o BackgroundPlotter para versões novas
            plotter = pv.Plotter(title=f"PrintFlow 3D - {os.path.basename(caminho)}")
            plotter.set_background("#2c3e50")
            
            # Converter trimesh para PyVista
            pv_mesh = pv.wrap(self.mesh_atual)
            
            # Adicionar peça com visual profissional
            plotter.add_mesh(pv_mesh, color="orange", show_edges=True, smooth_shading=True, specular=0.5)
            plotter.add_axes()
            plotter.view_isometric()
            
            # Abre a janela externa de forma independente
            plotter.show()
            
        except Exception as e:
            messagebox.showerror("Erro de Renderização", f"Ocorreu um problema ao abrir o motor 3D: {e}")
    ...
    def atualizar_lista_arquivos(self):
        ...
        for arq in [f for f in os.listdir(self.pasta_projetos) if f.endswith('.stl')]:
            ...
            ctk.CTkLabel(f, text=arq[:18], font=("Arial", 11)).pack(side="left", padx=5)
            
            # Botões
            ctk.CTkButton(f, text="🗑️", width=30, fg_color="#c0392b", command=lambda a=arq: self.deletar_stl(a)).pack(side="right", padx=2)
            ctk.CTkButton(f, text="👁", width=30, fg_color="#3498db", command=lambda a=arq: self.visualizar_stl(os.path.join(self.pasta_projetos, a))).pack(side="right", padx=2)
