...
class AbaEstoque(ctk.CTkScrollableFrame):
    def __init__(self, master, refresh_callback):
        ...
        # Eventos para cálculo e sincronização
    ...
    def preparar_reposicao(self):
        ...
        self.ent_saldo.insert(0, peso_novo)
        self.saldo_original = str(peso_novo)
        messagebox.showinfo("Modo Reposição", "Campos prontos para novo registro.")
    ...
    def mudar_modo(self, modo):
        ...
        if modo == "filamento":
            self.lbl_v2.configure(text="Cor:")
            ...
            self.lbl_v2.configure(text="Peso Total Bandeja (g):")
            self.lbl_v3.configure(text="Tempo Impressão (h):")
            self.lbl_v4.configure(text="Hora-Máquina (R$):")
            self.lbl_v5.configure(text="Qtd de Peças:")
            self.lbl_data.configure(text="---")
            self.ent_data.delete(0, 'end')
            self.ent_data.configure(state="disabled", fg_color="transparent")
    ...
    def salvar(self):
        ...
        try:
            ...
            if self.tabela_ativa == "filamento":
                ...
                data_c = self.ent_data.get()
                
                txt_s = self.ent_saldo.get().replace(",", ".")
                saldo_f = float(txt_s) if txt_s else v3

                if idx and str(saldo_f) != self.saldo_original:
                    if not messagebox.askyesno("Confirmar", "Deseja alterar o saldo manualmente?"): return
                ...
                v2_p = float(self.ent_v2.get().replace(",", ".") or 0)
                v5_p = int(self.ent_v5.get() or 1)
                if not idx:
    ...
    def calcular_dinamico(self):
        """ Sincroniza o saldo com o peso inicial e calcula o R$/g """
        if self.tabela_ativa == "filamento":
            p_str = self.ent_v3.get().replace(",", ".")
            c_str = self.ent_v5.get().replace(",", ".")
            
            # Sincroniza Saldo (Apenas se não for edição de item existente)
            if not self.ent_id.get():
                self.ent_saldo.delete(0, 'end')
                self.ent_saldo.insert(0, p_str)
    ...
    def refresh(self):
        ...
        for r in db.query("SELECT id, material, cor, quantidade_g, peso_atual_g, preco_por_g, custo_compra, data_compra FROM filamento"):
            self.tree_f.insert("", "end", values=tuple(r))
        for r in db.query("SELECT id, nome, peso_u, tempo_h, quantidade_produzida, hora_maq, preco_sugerido FROM produtos"):
    ...
    def criar_tabela(self, cols, tit):
        ...
        t = ttk.Treeview(t_frame, columns=cols, show="headings", height=6)
        t.pack(side="left", fill="x", expand=True)
        for c in cols: t.heading(c, text=c); t.column(c, width=80, anchor="center")
